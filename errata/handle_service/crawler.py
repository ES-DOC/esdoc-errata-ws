...
def crawler(input_handle, handle_client_instance):
    ...
    if aggregation_level == FILE:
        ...
        for dset in file_record.parents:
            output[dset.handle_string] = [file_record.parents[-1].errata, dset.id, dset.version, order_index]
        # logger.log('Lineage yielded {} for processors and {} for successors'.format(file_record.parents[0].lineage[0],
        #                                                                             file_record.parents[-1].lineage[1]))
        while file_record.parents[0].lineage[0]:
            ...
            logger.log('Processing predecessor #{} found from first parent #{}'.format(predecessor.id, dataset_record.id))
            try:
                ...
                for dset in file_record.parents:
                    output[dset.handle_string] = [predecessor.errata, dset.id, dset.version, order_index]
        ...
        while file_record.parents[-1].lineage[1]:
            ...
            logger.log('Processing successor #{} found from last parent #{}'.format(successor.id, dataset_record.id))
            try:
                ...
                list_of_files.append(file_record)
                for dset in file_record.parents:
                    output[dset.handle_string] = [successor.errata, dset.id, dset.version, order_index]
            except FileNotFoundInHandle as e:
                incomplete_retracing = True
                logger.log(e.message)
                break
